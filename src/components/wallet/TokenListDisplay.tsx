...
export function TokenListDisplay({ onCloseAction }: TokenListDisplayProps) {
  ...
  const getUserBalance = (faAddress: string) => {
    ...
    // Debug: Log the coinBalances structure
    if (coinBalances.length > 0) {
      ...
      console.log(
        "Available asset_types:",
        coinBalances.map((asset) => asset.asset_type)
      );
    }
    ...
    if (
      ...
      console.log(`Balance found for ${token.symbol}:`, {
        rawAmount: userAsset.amount,
        decimals,
        calculatedBalance: balance,
        assetType: userAsset.asset_type,
      });
    ...
    <div className="w-full h-full flex flex-col ">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
        <div className="flex items-center gap-3">
          <h2 className="text-white font-semibold text-lg">All Tokens</h2>
              ...
              {isLoadingCoinBalances ? (
                ...
                filteredTokens.map((token: Token) => {
                  ...
                  return (
                    <div
                      key={token.faAddress}
                      className={`bg-white/5 backdrop-blur-md rounded-lg p-4 ${userBalance ? "border-t-2 border-primary !bg-primary/10" : ""}`}
                    >
                      <div className="flex justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gray-700 rounded-full flex items-center justify-center overflow-hidden">
                            ...
                            <span
                              className={`text-lg ${token.logoUrl ? "hidden" : ""}`}
                            >
                              🔵
                            </span>
                          </div>
